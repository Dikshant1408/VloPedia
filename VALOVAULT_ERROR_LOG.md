...
| **13** | **Search Console** | `Crawled - currently not indexed` on `/skins/[uuid]` (e.g. `43e01e91-44d0-a819-852b-35b71673c648` Sandswept Sheriff) | Skin pages rendered thin client-only HTML without server-rendered text, specs, variants breakdown, or FAQ schema, causing Google to deem them low-value content. | Overhauled `src/app/skins/[slug]/page.tsx` with rich server-rendered HTML: AnswerBox, Skin Specifications table, Chroma Variants gallery, Radianite Level progression, Product & FAQPage JSON-LD, and parent weapon links. | ✅ **RESOLVED** |
